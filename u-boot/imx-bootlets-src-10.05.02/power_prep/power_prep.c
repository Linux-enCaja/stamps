...
#define MAX_4P2_CAP_CHARGE_TIME 10000
...
//#define MXS_VBUS_CURRENT_DRAW

#define VBUS_CURRENT_LIMIT 	500
...
int _start( void )
{
	...
	printf("\r\nbooting...\r\n");
	hw_digctl_MicrosecondWait(1000000);

	printf("\r\nPowerPrep start initialize power...\r\n");

	//PowerPrep_Enable4p2();

	ddi_power_SetVddio(3300, 3000);

	hw_digctl_MicrosecondWait(100000);

//	HW_POWER_VDDDCTRL.B.LINREG_OFFSET = 2;
//	HW_POWER_VDDACTRL.B.LINREG_OFFSET = 2;
//	HW_POWER_VDDIOCTRL.B.LINREG_OFFSET = 2;
...
void PowerPrep_Enable4p2( void )
{
	...
	*/
	...
	HW_POWER_VDDDCTRL_CLR(BM_POWER_VDDDCTRL_PWDN_BRNOUT);
	...
	else
	{
...

		{
			...
			if(HW_POWER_STS.B.DCDC_4P2_BO == 0)
			{
				...
				BOOTUP_CHARGE_4P2_CURRENT_LIMIT);
				hw_digctl_MicrosecondWait(MAX_4P2_CAP_CHARGE_TIME);
		...
		printf("VDD5V is sourced from VBUS,limit the current to %d mA\r\n", VBUS_CURRENT_LIMIT);
		HW_POWER_5VCTRL.B.CHARGE_4P2_ILIMIT =
